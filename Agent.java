...
public abstract class Agent {
	...
	public List<String> pathFrom(TreeNode current) {
		...
		List<String> actionList = new ArrayList<String>();//this is the initial list of actions
		List<String> reservedList = new ArrayList<String>();//this will be a temporary list that we will use to reserved and get the path of actions
		
		while(current.getParent() != null) {//this loop will run while we are not at the root node which would have no parent 	
			actionList.add(current.getAction());//for each action available i'll just add it to the list 
			current = current.getParent();//setting the current the its parent node above it to traverse up 
		}
		
		for(int i = actionList.size() -1; i>= 0; i--) {//starting from the end of the action since we traversed bottom up 
			reservedList.add(actionList.get(i));//add into the reserved list 
		}
		
		actionList = reservedList;//making action list equal to the new reserved list 
